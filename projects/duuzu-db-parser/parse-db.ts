...
const INPUT_FILE_NAME: string = 'keybpm_v10.txt';
...
export async function writeSongDetailsToFile(details: SongDetails[]): Promise<void> {
    fs.writeFile(`./${OUTPUT_FILE_NAME}`, JSON.stringify(details, undefined, 4), (err: unknown) => {
        if (err) throw err;
    });
}
